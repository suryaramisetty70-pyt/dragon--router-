...
**Dragon Router** — unified AI proxy/router. One endpoint, 237 LLM providers, auto-fallback.
...
Dragon Router has three related but distinct temporary-failure mechanisms. Keep their
...
repo** instead of polluting the main Dragon Router tree.
...
- **Runtime**: Node.js ≥22.0.0 <23 || ≥24.0.0 <27, ES Modules. This is the **only** runtime for the published `dragonrouter` CLI, the server, and the test suites (`node:test` + vitest) — `engines.node` is authoritative and end users never need Bun.
...
- **Data directory**: `DATA_DIR` env var, defaults to `~/.dragonrouter/`
...
Dragon Router has **~48 quality-gate scripts** (`scripts/check/` + `scripts/quality/`) wired
