...
async function main() {
  ...
  if (fs.existsSync(modelsRoute)) {
    ...
    content = content.replace(
      /<<<<<<< HEAD\r?\n=======\r?\nimport \{ sanitizeErrorMessage \} from "@dragonrouter\/open-sse\/utils\/error";\r?\nimport \{ getStaticQoderModels \} from "@dragonrouter\/open-sse\/services\/qoderCli\.ts";\r?\n>>>>>>> release\/v3\.8\.4/g,
      'import { sanitizeErrorMessage } from "@dragonrouter/open-sse/utils/error";\nimport { getStaticQoderModels } from "@dragonrouter/open-sse/services/qoderCli.ts";'
    );
...
# arbitrary code at install time. Dragon Router's own postinstall (
