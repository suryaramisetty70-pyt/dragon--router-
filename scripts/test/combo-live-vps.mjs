 ...
 *
 * Phase-3 VPS HTTP scenario driver for Dragon Router combo routing.
...
// Known approximate input cost ($/M tokens) from Dragon Router's default-pricing constants.
...
//
// Dragon Router's sortModelsByCost uses getPricingForModel which merges
...
async function scenarioCostOptimized(healthy) {
  ...
  try {
    ...
    // Both direct match and provider-substring match are accepted since Dragon Router
...
//   404, Dragon Router calls recordProviderCooldown(providerA, undefined) — a provider-
...
//   1. SSH to read STORAGE_ENCRYPTION_KEY from /root/.dragonrouter/.env
//   2. Encrypt a wrong API key using scryptSync(key,"dragonrouter-field-encryption-v1",32)+AES-256-GCM
...
// ---------------------------------------------------------------------------
async function main() {
  console.log("=== Dragon Router combo-live-vps scenario driver ===");
