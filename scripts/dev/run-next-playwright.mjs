...
const testServerEnv = {
  ...
  ...(bootstrapMode === "open"
    ? {
        INITIAL_PASSWORD: "",
        DRAGONROUTER_E2E_PASSWORD: "",
        DRAGONROUTER_API_KEY: "",
      }
    : {
        INITIAL_PASSWORD: playwrightPassword,
        DRAGONROUTER_E2E_PASSWORD: playwrightPassword,
      }),
  ...(process.env.DRAGONROUTER_USE_TURBOPACK
    ? {
        DRAGONROUTER_USE_TURBOPACK: process.env.DRAGONROUTER_USE_TURBOPACK,
      }
    : {}),
};
...
export function shouldUseWebpackForPlaywrightDev({ mode, env }) {
  ...
  return mode === "dev" && env.DRAGONROUTER_USE_TURBOPACK === "0";
}
...
async function runBuildForStart() {
  ...
  if (process.env.DRAGONROUTER_PLAYWRIGHT_SKIP_BUILD === "1") return;
