#!/usr/bin/env node
/**
 * Dragon Router — Environment Sync
 *
...
const CRYPTO_SECRETS = {
  ...
  // Generated in bin/dragonrouter.mjs:ensureStorageEncryptionKey() and persisted to
  // ~/.dragonrouter/.env to survive across upgrades. This prevents credential loss
  // when upgrading Dragon Router (issue #1622).
  MACHINE_ID_SALT: () => `dragonrouter-${randomBytes(8).toString("hex")}`,
};
 ...
 * It's generated at server startup in bin/dragonrouter.mjs and persisted to
 * ~/.dragonrouter/.env to survive across upgrades.
...
function resolveDataDir(env = process.env) {
  ...
  if (process.platform === "win32") {
    ...
    return join(appData, "dragonrouter");
  }

  const xdg = env.XDG_CONFIG_HOME?.trim();
  if (xdg) return join(resolve(xdg), "dragonrouter");

  return join(homedir(), ".dragonrouter");
}
        ...
        if (ENCRYPTION_BOUND_KEYS.has(key) && dbHasEncrypted) {
          log(
            `⚠️  ${key} NOT generated — encrypted credentials exist in DB. ` +
              `Restore your previous key via ~/.dragonrouter/server.env, ~/.dragonrouter/.env, ` +
