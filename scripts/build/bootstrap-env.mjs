#!/usr/bin/env node
/**
 * Dragon Router — Zero-Config Bootstrap
 *
 ...
 *   3. Preferred config .env  (DATA_DIR/.env -> ~/.dragonrouter/.env -> ./.env)
...
function writeEnvFile(filePath, env) {
  const lines = [
    "# Auto-generated by Dragon Router bootstrap — do not delete",
...
export function bootstrapEnv({ dataDirOverride, quiet = false } = {}) {
  ...
  // This keeps run-next / run-standalone consistent with `bin/dragonrouter.mjs`.
  //
  ...
  if (needsPersist) {
    merged.DRAGONROUTER_BOOTSTRAPPED = "true";
  }
        ...
        if (row) {
          ...
          if (ciphertext?.startsWith("enc:v1:")) {
            ...
            if (parts.length >= 5) {
              ...
              // Legacy salt (fallback): scryptSync(secret, "dragonrouter-field-encryption-v1", 32)
              const legacySalt = "dragonrouter-field-encryption-v1";
              ...
              if (!keyMatched) {
                ...
                log(
                  "   Either restore your previous key via ~/.dragonrouter/server.env or ~/.dragonrouter/.env,"
                );
                log(
                  "   or run: dragonrouter reset-encrypted-columns --force  (wipes credentials, keeps provider config)"
                );
              }
            }
          }
