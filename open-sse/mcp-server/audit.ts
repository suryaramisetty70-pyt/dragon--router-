...
declare global {
  var __dragon_routerMcpAuditDb: AuditDatabase | null | undefined;
}
...
function getCachedAuditDb(): AuditDatabase | null {
  return globalThis.__dragon_routerMcpAuditDb ?? null;
}

function setCachedAuditDb(database: AuditDatabase | null): void {
  globalThis.__dragon_routerMcpAuditDb = database;
}
  ...
  if (!nodeSqliteFallbackAvailable()) {
    console.error(
      `[MCP Audit] better-sqlite3 native binding unavailable and Node ${process.version} ` +
        ...
        "`npm rebuild better-sqlite3` in the dragon-router install root."
    );
    return null;
  }
...
async function getDb(): Promise<AuditDatabase | null> {
  ...
  try {
    ...
    const dbPath = process.env.DATA_DIR
      ...
      : join(homedir(), ".dragon-router", "storage.sqlite");
...
export async function logToolCall(
  ...
  try {
    ...
    const apiKeyId = process.env.DRAGON_ROUTER_API_KEY_ID || null;
