...
/** Renders the X-DragonRouter-Compression response header value. */
export function formatCompressionMeta(plan: DerivedPlan): string {
  return `${plan.mode}; source=${plan.source ?? "off"}`;
}

/**
 * Builds the annotation suffix for X-DragonRouter-Compression from compression stats.
 ...
 * ASCII-only: this string is appended to the X-DragonRouter-Compression HTTP header,
