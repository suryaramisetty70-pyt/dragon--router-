 ...
 * `stream`. Without this hint, DragonRouter previously routed those requests with
...
export function resolveStreamFlag(
  ...
  // DragonRouter accumulates the provider stream and converts it to JSON for the client
  ...
  // DragonRouter fallback would force SSE upstream and fail JSON-only providers as
  ...
  // No explicit stream param — preserve DragonRouter's streaming default unless
