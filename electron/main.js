/**
 * DragonRouter Electron Desktop App - Main Process
 *
...
function resolveNodeExecutable(env = process.env) {
  ...
  // (e.g. DragonRouter.app/Contents/MacOS/DragonRouter). Spawning it with
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    if (Notification.isSupported()) {
      const notification = new Notification({
        title: "DragonRouter Update Ready",
...
function startNextServer() {
  ...
  if (changed) {
    ...
    try {
      ...
      const lines = [
        "# Auto-generated by DragonRouter bootstrap",
        "",
  ...
  nextServer.stdout?.on("data", (data) => {
    ...
    if (text.includes("Ready") || text.includes("started") || text.includes("listening")) {
      ...
      if (isHeadless && !global.loggedHeadlessReady) {
        ...
        console.log("\n\x1b[32m✔ DragonRouter Headless CLI Server is ready and listening!\x1b[0m");
  ...
  try {
    ...
    const desktopFileContent =
      [
        ...
        "Type=Application",
        "Name=DragonRouter",
        "Comment=DragonRouter Desktop Client",
