 ...
 *
 * It talks ONLY to Google (no DragonRouter server needed locally), so it works even
...
function defaultStartServer(preferredPort) {
  return new Promise((resolve, reject) => {
    ...
    const server = createServer((req, res) => {
      ...
      res.end(
        "<!doctype html><meta charset=utf-8><title>DragonRouter</title>" +
...
export async function runAntigravityLogin(opts = {}, deps = {}) {
  ...
  print(
    "\n" +
      ...
      'DragonRouter dashboard: Providers → Antigravity → Connect → "Paste credentials".\n' +
...
export function registerLogin(program) {
  const login = program
    .command("login")
    .description("Local OAuth helpers for remote DragonRouter installs (run on your own machine)");
