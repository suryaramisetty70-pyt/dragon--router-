 ...
 * is referenced by env var (DRAGON_ROUTER_API_KEY) — never written to disk.
 */
...
function buildCodexProfile(modelId, ctx) {
  const lines = [
    ...
    `# ${modelId} — generated by 'dragon-router configure codex'`,
    `model                          = "${modelId}"`,
...
async function configureCodex(modelId, ctxWindow, opts) {
  ...
  printInfo("Prereq: ~/.codex/config.toml must define the [model_providers.dragon-router] block");
