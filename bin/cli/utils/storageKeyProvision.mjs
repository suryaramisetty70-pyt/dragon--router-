 ...
 *
 * Purely informational invocations must NOT create `~/.dragon-router/.env` or write
 * a key — they never touch encrypted storage. Generating a 32-byte key and a
 * `.env` file just to print `dragon-router --version` (or `--help`) is a surprising
 ...
 *
 * Returns TRUE for everything else, INCLUDING a bare `dragon-router` (no args) — the
...
export function shouldProvisionStorageKey(argv) {
  ...
  // Bare `dragon-router` runs the default `serve` command → must provision.
