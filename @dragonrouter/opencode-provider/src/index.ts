 ...
 *     });
 *     // config -> { $schema, provider: { dragonrouter: { npm, name, options, models } } }
 *     ```
 *
...
export const DRAGONROUTER_PROVIDER_NPM = "@ai-sdk/openai-compatible" as const;
 ...
 *
 * Mirrors the capability shape used by Alph4d0g/opencode-dragonrouter-auth
...
export interface DragonRouterProviderOptions {
   ...
   * `DRAGONROUTER_DEFAULT_MODEL_CAPABILITIES` for ids in the default catalog;
   * for custom ids the override is used verbatim.
   */
  modelCapabilities?: Record<string, ModelCapabilities>;
  /**
   * Optional per-model context-length overrides (tokens). Takes precedence
   * over the static `DRAGONROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS` map but is
   ...
   * Emitted as `"dragonrouter/<id>"`. When omitted the key is not written.
   */
  model?: string;
  /**
...
/** Per-model entry written under `provider.dragonrouter.models[id]`. */
...
export interface OpenCodeConfigDocument {
  ...
  /** Primary model for OpenCode, e.g. `"dragonrouter/claude-sonnet-4-5-thinking"`. */
  model?: string;
  /** Secondary / cheap model for OpenCode, e.g. `"dragonrouter/gemini-3-flash"`. */
  small_model?: string;
  provider: {
    [DRAGONROUTER_PROVIDER_KEY]: OpenCodeProviderEntry;
  };
}

function requireNonEmpty(value: unknown, field: string): string {
  if (typeof value !== "string") {
    throw new TypeError(`@dragonrouter/opencode-provider: ${field} must be a string`);
  }
  const trimmed = value.trim();
  if (!trimmed) {
    throw new Error(`@dragonrouter/opencode-provider: ${field} is required and cannot be empty`);
  }
  return trimmed;
}
...
export function normalizeBaseURL(rawBaseURL: string): string {
  ...
  try {
    ...
    throw new Error(
      `@dragonrouter/opencode-provider: baseURL is not a valid URL: ${JSON.stringify(rawBaseURL)}`
    );
  }
  let base = trimmed;
  let end = base.length;
...
export function createDragonRouterProvider(
  ...
  for (const raw of modelList) {
    ...
    seen.add(id);
    const defaults = DRAGONROUTER_DEFAULT_MODEL_CAPABILITIES[id] ?? {};
    ...
    const rawContextLength =
      ...
      DRAGONROUTER_DEFAULT_MODEL_CONTEXT_LENGTHS[id];
    const contextLength =
...
/**
 * Build a full OpenCode config document (with `$schema` + `provider.dragonrouter`).
...
export function buildDragonRouterOpenCodeConfig(
  ...
  if (options.model !== undefined) {
    ...
    if (id) doc.model = `${DRAGONROUTER_PROVIDER_KEY}/${id}`;
  }

  if (options.smallModel !== undefined) {
    const id = options.smallModel.trim();
    if (id) doc.small_model = `${DRAGONROUTER_PROVIDER_KEY}/${id}`;
  }
 ...
 * present are not removed. If `existing.provider.dragonrouter` already exists it
...
export function mergeIntoExistingConfig(
  ...
  merged.provider = {
    ...existingProvider,
    [DRAGONROUTER_PROVIDER_KEY]: partial.provider[DRAGONROUTER_PROVIDER_KEY],
  };
 ...
 */
export const DRAGONROUTER_MCP_DEFAULT_SCOPES = [
  "read:health",
  "read:combos",
  "read:quota",
  "read:usage",
...
export interface DragonRouterMCPOptions {
   ...
   * When supplied it is forwarded as `DRAGONROUTER_MANAGEMENT_API_KEY`.
   */
  managementApiKey?: string;
  /**
   * Comma-separated scope list passed as `DRAGONROUTER_MCP_SCOPES`.
...
/**
 * Build the `mcp.servers.dragonrouter` entry for an OpenCode config document.
 *
 * @example
 * ```ts
...
export function createDragonRouterMCPEntry(
  ...
  if (options.managementApiKey !== undefined) {
    ...
    if (mgmtKey) env.DRAGONROUTER_MANAGEMENT_API_KEY = mgmtKey;
  }

  if (options.scopes !== undefined && options.scopes.length > 0) {
    env.DRAGONROUTER_MCP_ENFORCE_SCOPES = "true";
    env.DRAGONROUTER_MCP_SCOPES = options.scopes.join(",");
  }
...
async function fetchJSON<T>(url: string, apiKey: string, timeoutMs: number): Promise<T> {
  ...
  try {
    ...
    throw new Error(`@dragonrouter/opencode-provider: request to ${url} failed: ${message}`);
  } finally {
    clearTimeout(timer);
  }
}
...
function buildAgentEntry(role: DragonRouterAgentRole): OpenCodeAgentEntry | undefined {
  ...
  const entry: OpenCodeAgentEntry = { model: `${DRAGONROUTER_PROVIDER_KEY}/${modelId}` };
