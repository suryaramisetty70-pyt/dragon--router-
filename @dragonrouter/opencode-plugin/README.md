...
> **Recommended way to use Dragon Router with OpenCode.** Pulls a live model catalog from `/v1/models` (including `-low`/`-medium`/`-high`/`-thinking` variants as first-class IDs), aggregates combos via `/api/combos` using a least-common-denominator capability/limit join, sanitizes Gemini tool schemas in flight, and supports multiple side-by-side Dragon Router instances out of the box.

## Why this and not `@dragonrouter/opencode-provider`?

`@dragonrouter/opencode-provider` is the legacy config-generator package — it writes a frozen `provider.dragonrouter` block into `opencode.json` with a **hardcoded list of 8 models** ([`DRAGONROUTER_DEFAULT_OPENCODE_MODELS`](https://github.com/diegosouzapw/Dragon Router/blob/main/%40dragonrouter/opencode-provider/src/index.ts#L48-L56)). It works on the CLI but in the **OpenCode Desktop / Web** builds (Tauri / Electron) the runtime re-runs the model picker and the static block surfaces only a few of those — and they drift behind the live Dragon Router catalog.
...
- Emitting the provider block **dynamically** in the plugin's `config`/`provider` hook — so `opencode.json` only needs the plugin entry, not a static `provider.dragonrouter`
- Re-fetching on a configurable TTL (default 5 min), so new models / combo changes in the Dragon Router UI appear without restarting OpenCode
...
The plugin ships **pre-built inside the `dragonrouter` npm package** since v3.8.23.
If you have Dragon Router installed, the plugin is already on disk:

```sh
# 1. One command — copy the plugin into OpenCode and update opencode.json
dragonrouter setup opencode --auth

# 2. Follow the interactive prompt to enter your Dragon Router API key
# 3. Restart OpenCode — /models lists the full live catalog
```

The `--auth` flag runs `opencode auth login --provider dragonrouter` automatically.
Use `--base-url` to point at a non-default Dragon Router address:

```sh
dragonrouter setup opencode --base-url https://or.example.com --auth
```

### What it does

1. Locates the bundled plugin inside the dragonrouter installation
2. Copies `dist/` + `package.json` to `~/.config/opencode/plugins/dragonrouter/`
...
`@dragonrouter/opencode-provider` or the legacy `opencode-dragonrouter-auth` package are
automatically cleaned up.

### Manual install (without dragonrouter CLI)

If you cannot run `dragonrouter setup opencode` (local dev, CI, air-gapped), reference
the built artifact directly:

```sh
cd @dragonrouter/opencode-plugin && npm run build && npm pack
# then extract into ~/.config/opencode/plugins/dragonrouter-opencode-plugin/
```
...
// opencode.json
{
  ...
  "plugin": [
    [
      "./plugins/dragonrouter-opencode-plugin/dist/index.js",
      {
...
```sh
opencode auth login --provider dragonrouter
# prompts for the Dragon Router API key, writes to ~/.local/share/opencode/auth.json
```

> ⚠ Use the `--provider` flag explicitly. `opencode auth login dragonrouter` is parsed as a positional `url` argument by current OC releases (≤1.15.5) and fails with `fetch() URL is invalid`. Tracked upstream.

Restart OpenCode. `/models` lists the full live catalog. Variants (`-low`, `-medium`, `-high`, `-thinking`) and combos appear as first-class IDs — Dragon Router is the source of truth, no client-side synthesis.
...
cd /path/to/Dragon Router/@dragonrouter/opencode-plugin
npm run build
npm pack
# produces dragonrouter-opencode-plugin-0.1.0.tgz

# 2. Extract one copy per Dragon Router endpoint
mkdir -p ~/.config/opencode/plugins/dragonrouter-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/dragonrouter-opencode-plugin-preprod
tar -xzf dragonrouter-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/dragonrouter-opencode-plugin-prod    --strip-components=1
tar -xzf dragonrouter-opencode-plugin-0.1.0.tgz -C ~/.config/opencode/plugins/dragonrouter-opencode-plugin-preprod --strip-components=1
```
...
```jsonc
{
  ...
  "plugin": [
    [
      "./plugins/dragonrouter-opencode-plugin-prod/dist/index.js",
      {
        "providerId": "dragonrouter",
        "displayName": "Dragon Router",
        "baseURL": "https://or.example.com",
      },
    ],
    [
      "./plugins/dragonrouter-opencode-plugin-preprod/dist/index.js",
      {
        "providerId": "dragonrouter-preprod",
        "displayName": "Dragon Router Preprod",
...
```sh
opencode auth login --provider dragonrouter
opencode auth login --provider dragonrouter-preprod
```
...
### After publish (`@dragonrouter/opencode-plugin` npm)
...
mkdir -p ~/.config/opencode/plugins/dragonrouter-opencode-plugin-prod
mkdir -p ~/.config/opencode/plugins/dragonrouter-opencode-plugin-preprod
npm install --prefix ~/.config/opencode/plugins/dragonrouter-opencode-plugin-prod    @dragonrouter/opencode-plugin
npm install --prefix ~/.config/opencode/plugins/dragonrouter-opencode-plugin-preprod @dragonrouter/opencode-plugin
```

`opencode.json` paths become `./plugins/dragonrouter-opencode-plugin-prod/node_modules/@dragonrouter/opencode-plugin/dist/index.js` (and the preprod equivalent).
...
| Variants pass-through                       | `-low`/`-medium`/`-high`/`-thinking` ship as first-class IDs from Dragon Router (no client synthesis)                                                                                                                                                                                                                                                                                                           | `provider.models`            |
...
| --------------- | -------- | ------------------------------------------ | ---------------------------------------------------------- |
| `providerId`    | `string` | `"dragonrouter"`                              | OpenCode provider id; must be unique across plugin entries |
| `displayName`   | `string` | `"Dragon Router"` or `Dragon Router (<id>)`        | Label in the OC UI                                         |
| `modelCacheTtl` | `number` | `300000` (5 min)                           | `/v1/models` TTL in ms                                     |
| `baseURL`       | `string` | resolved from `auth.json` after `/connect` | Override Dragon Router base URL                                |
...
| `diskCache`           | `boolean` | `true`  | Persist the last successful `/v1/models` + `/api/combos` + enrichment + connections + compression snapshot to `${OPENCODE_DATA_DIR ?? ~/.local/share/opencode}/plugins/dragonrouter-<providerId>.json`. On a subsequent cold start where `/v1/models` throws (network down / IP whitelist drop / 5xx) the static block hydrates from the snapshot so OC's model picker survives offline. Soft-fail on read/write — never blocks publishing.                                                                                                                                     |
...
With `mcpAutoEmit: true`, the plugin synthesises an `mcp.dragonrouter` entry equivalent to a manual:
...
If you want a narrower-scoped Bearer for MCP (different from the chat/inference key), set `features.mcpToken`. Operator overrides win: if you already set `mcp.dragonrouter` in `opencode.json`, the plugin will not overwrite it.
...
- `usableOnly: true` drops models whose canonical provider has no healthy connection in your Dragon Router instance — your `/models` picker stays focused on what you can actually call.
- `diskCache: true` (default) writes a snapshot to `${OPENCODE_DATA_DIR}/plugins/dragonrouter-<providerId>.json` on every healthy refresh. On a cold start where `/v1/models` is unreachable (laptop offline, IP whitelist drop), the snapshot hydrates the static block so OC still shows the catalog instead of a stub.
- `compressionMetadata: true` annotates combo display names with their pipeline using traffic-light emoji for intensity (e.g. `Combo: claude-primary [rtk🟡 → caveman🟠]`) so the picker advertises which compression each combo applies and how heavy it is at a glance. Palette: 🟢 lite/minimal · 🟡 standard · 🟠 aggressive/full · 🔴 ultra. Unknown intensities fall through to raw text (`[rtk:custom-thing]`) so the plugin never hides a value Dragon Router knows but the plugin doesn't.
...
## Comparison vs `@dragonrouter/opencode-provider`

[`@dragonrouter/opencode-provider`](https://github.com/diegosouzapw/Dragon Router/tree/main/%40dragonrouter/opencode-provider) is the existing config-generator package — it writes a frozen `provider.<id>` block into `opencode.json` at build time. This plugin is the runtime integration.

|                   | `@dragonrouter/opencode-plugin` (this) | `@dragonrouter/opencode-provider`    |
| ----------------- | ----------------------------------- | --------------------------------- |
