...
test("createDragonRouterProviderHook: custom providerId binds to hook.id (multi-instance)", () => {
  ...
  assert.equal(a.id, "opencode-dragonrouter-preprod");
  assert.equal(b.id, "opencode-dragonrouter-local");
});
...
test("models: maps a sample /v1/models entry to ModelV2 (sanity)", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-abc") as never });
  const claude = out["opencode-dragonrouter/claude-primary"];
  ...
  assert.equal(claude.id, "opencode-dragonrouter/claude-primary");
  assert.equal(claude.name, "claude-primary");
  assert.equal(claude.providerID, "opencode-dragonrouter");
