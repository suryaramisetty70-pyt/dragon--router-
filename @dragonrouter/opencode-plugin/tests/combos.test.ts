...
test("mapComboToModelV2: api block matches providerId + baseURL", () => {
  const m = mapComboToModelV2(
    { id: "c" },
    [MODEL_PRIMARY],
    "dragonrouter-preprod",
    "https://or-preprod.example.com/v1"
  );
  assert.equal(m.providerID, "dragonrouter-preprod");
...
test("models() returns combo entries merged into the map", async () => {
  ...
  assert.ok(out["opencode-dragonrouter/claude-primary"]);
  assert.ok(out["opencode-dragonrouter/claude-secondary"]);
  assert.ok(out["opencode-dragonrouter/gemini-3-flash"]);
  assert.ok(out["opencode-dragonrouter/claude-tier"]);

  const combo = out["opencode-dragonrouter/claude-tier"];
  assert.equal(combo.name, "Claude Tier");
  assert.equal(combo.providerID, "opencode-dragonrouter");
...
test("models(): combo with unknown member ids degrades to all-false LCD posture", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-dragonrouter/phantom-combo"]);
  // With zero resolvable members, LCD = all-false (defensive posture).
  assert.equal(out["opencode-dragonrouter/phantom-combo"].capabilities.toolcall, false);
  assert.equal(out["opencode-dragonrouter/phantom-combo"].capabilities.reasoning, false);
  assert.equal(out["opencode-dragonrouter/phantom-combo"].limit.context, 0);
});
...
test("models(): hidden combos are excluded from the map", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  assert.ok(out["opencode-dragonrouter/visible"]);
  assert.ok(!out["opencode-dragonrouter/hidden"], "hidden combo must be omitted");
});
...
test("models(): combo name exactly matches raw model id → raw deleted, raw deleted, no warn", async () => {
  ...
  assert.ok(out["opencode-dragonrouter/claude-primary"], "combo surfaces under prefixed key");
  assert.equal(out["opencode-dragonrouter/claude-primary"].name, "claude-primary");
...
test("models(): two combos with same slug → second gets disambiguator suffix", async () => {
  ...
  assert.ok(out["opencode-dragonrouter/claude"], "first combo at prefixed slug");
  assert.ok(out["opencode-dragonrouter/claude-uuid"], "second combo disambiguated by id prefix");
});
...
test("models(): combos cached + reused within TTL (one combo fetch per TTL window)", async () => {
  ...
  assert.ok(second["opencode-dragonrouter/claude-tier"]);
});
...
test("models(): nested combo-ref context is the min of nested + raw members", async () => {
  ...
  const out = await hook.models!({} as never, { auth: apiAuth("sk-z") as never });
  const masterLight = out["opencode-dragonrouter/master-light"];
