 ...
 *
 *   "plugin": [
 *     ["@dragonrouter/opencode-plugin", {"providerId": "dragonrouter-prod",    "baseURL": "https://or.example/v1"}],
 *     ["@dragonrouter/opencode-plugin", {"providerId": "dragonrouter-preprod", "baseURL": "https://or-preprod.example/v1"}]
 *   ]
 *
 * Assertions:
...
test("multi-instance: two plugin invocations bind to their own providerId", async () => {
  ...
  assert.equal(a.auth?.provider, "opencode-dragonrouter-prod");
  assert.equal(b.auth?.provider, "opencode-dragonrouter-preprod");
});
