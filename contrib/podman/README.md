...
Run Dragon Router with Podman via **Quadlet** (systemd integration) or **podman compose**.
...
```bash
cd /path/to/dragonrouter
podman build --target runner-base -t dragonrouter:base .
# For web-cookie providers (gemini-web, claude-web, claude-turnstile):
podman build --target runner-web -t dragonrouter:web .
# For CLI tool support:
podman build --target runner-cli -t dragonrouter:cli .
```

### 2. Copy Quadlet files to the systemd directory

```bash
mkdir -p ~/.config/containers/systemd/dragonrouter
cp contrib/podman/*.container ~/.config/containers/systemd/dragonrouter/
cp contrib/podman/*.network ~/.config/containers/systemd/dragonrouter/
cp contrib/podman/*.volume ~/.config/containers/systemd/dragonrouter/
```

### 3. Mount the project .env for secrets

Edit `~/.config/containers/systemd/dragonrouter/dragonrouter.container` and
uncomment/replace the `EnvironmentFile` line with the absolute path to
your project `.env`:

```
EnvironmentFile=/home/USER/code/docker/Dragon Router/.env
```
...
systemctl --user start dragonrouter-redis
systemctl --user start dragonrouter
```

### 5. Verify

```bash
systemctl --user status dragonrouter
...
```bash
journalctl --user -u dragonrouter -f
```

### 6. Enable on boot

```bash
systemctl --user enable dragonrouter-redis
systemctl --user enable dragonrouter
```
