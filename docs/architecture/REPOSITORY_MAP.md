...
> Last updated: 2026-06-28 — Dragon Router v3.8.40
>
...
├── @dragonrouter/           # Publishable companion packages (opencode-plugin, opencode-provider)
├── skills/               # CLI/agent skill packs (cli-* + omni-* + config-codex-cli)
├── examples/             # Sample plugins + dragonrouter-cmd-hello starter
...
Separate npm workspace (`@dragonrouter/open-sse`). Handles request processing + provider execution.
...
- **`_mono_repo/`** — historic subprojects (dragonrouterCloud, dragonrouterSite, vscode-extension)
