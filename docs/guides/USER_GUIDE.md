...
models. Haiku models do not accept the `max` effort tier, so Dragon Router downgrades that
...
Dragon Router clamps it down to that cap before sending the upstream request.
...
Dragon Router is both an **MCP server** (Model Context Protocol) and an **A2A server** (Agent-to-Agent JSON-RPC 2.0). Any MCP-compatible IDE or agent host can call Dragon Router tools directly — no extra wrapper required.

### MCP transports

- **SSE**: `http://localhost:20128/api/mcp/sse`
- **Streamable HTTP**: `http://localhost:20128/api/mcp/stream`
- **stdio**: `dragonrouter --mcp` (for IDE plugins that prefer stdio)
...
Dragon Router exposes an extensible **skill framework** (`src/lib/skills/`) so agents and the A2A endpoint can run domain-specific routines (e.g. `code-review`, `summarize`, `extract-facts`, `web-research`).
...
Dragon Router persists **long-term conversational memory** with hybrid retrieval:
...
Subscribe to Dragon Router events for real-time monitoring and automation.
...
- Every payload includes `X-Dragon Router-Signature` (HMAC-SHA256) for verification
...
Dragon Router integrates with cloud coding agents (**OpenAI Codex Cloud**, **Devin**, **Jules**, **Antigravity**) so you can dispatch long-running tasks from the same dashboard that handles your local routing.

- Create tasks in **Dashboard → Cloud Agents** or via `POST /api/v1/agents/tasks`
- Track status, logs, and artifacts per task
- Bring-your-own API key per provider — credentials never leave the Dragon Router instance
...
You can manage every Dragon Router resource (providers, combos, keys, settings) over HTTP using a **Bearer key with the `manage` scope**.
...
Dragon Router ships an internal CLI (`dragonrouter …`) for setup, diagnostics, and runtime control. This is **separate from the "CLI Tools" page in the dashboard**, which configures third-party CLIs (Claude Code, Cursor, Codex, Cline, …) so they can talk to Dragon Router.

```bash
dragonrouter setup                    # Interactive wizard (password, providers, combos)
dragonrouter setup --non-interactive  # CI-friendly
dragonrouter doctor                   # Health diagnostics (data dir, DB, providers, ports)
dragonrouter providers available      # List supported providers
dragonrouter providers list           # List configured connections
dragonrouter providers test <id>      # Live test a provider connection
dragonrouter combos list              # List combos
dragonrouter combos switch <name>     # Set default combo
dragonrouter models                   # List available models (--json, --search)
dragonrouter keys add | list | remove # Manage API keys from the terminal
dragonrouter backup                   # Snapshot config + DB
dragonrouter restore [<timestamp>]    # Restore from a snapshot
dragonrouter health                   # Detailed health (breakers, cache, memory)
dragonrouter quota                    # Provider quota usage
dragonrouter mcp status               # MCP server status
dragonrouter a2a status               # A2A server status
dragonrouter tunnel list|create|stop  # Cloudflare/Tailscale/ngrok tunnels
dragonrouter reset-password           # Reset the admin password
dragonrouter --mcp                    # Start MCP server over stdio
dragonrouter --port 3000              # Start the server on a custom port
```

Tip: pair `dragonrouter doctor --json` with your monitoring tool to alert on unhealthy provider connections.
