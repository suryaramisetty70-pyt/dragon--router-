...
Dragon Router ships a cross-platform desktop app (Windows / macOS / Linux) built on
...
- Windows: `%APPDATA%\dragonrouter`
- Linux: `$XDG_CONFIG_HOME/dragonrouter` or `~/.dragonrouter`
- macOS: `~/.dragonrouter`
...
- Close button minimizes to tray; the tray menu has **Open Dragon Router**, **Open Dashboard** (external browser), **Server Port** submenu, **Check for Updates**, **Quit**.
...
- `connect-src 'self' http://localhost:* http://127.0.0.1:* ws://localhost:* ws://127.0.0.1:* https://*.dragonrouter.online https://*.dragonrouter.dev`
- Dev mode adds `'unsafe-eval'` to `script-src` only

## Auto-update

Uses `electron-updater` with the GitHub provider (`diegosouzapw/Dragon Router`).
...
- `Dragon Router Setup X.Y.Z.exe`, `Dragon Router-X.Y.Z-portable.exe` (Windows)
- `Dragon Router-X.Y.Z-mac.dmg`, `Dragon Router-X.Y.Z-arm64-mac.dmg` (macOS)
- `Dragon Router-X.Y.Z.AppImage`, `dragonrouter-desktop_X.Y.Z_amd64.deb` (Linux)

Releases are published to GitHub Releases (`diegosouzapw/Dragon Router`), which is also where `electron-updater` checks for new versions.
