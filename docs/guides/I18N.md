...
Dragon Router supports **42 languages** with full dashboard UI translation, translated documentation, and RTL support for Arabic and Hebrew.
...
Dragon Router uses a hash-based incremental translator for docs, backed by an
OpenAI-compatible LLM endpoint (typically `cx/gpt-5.4-mini` through Dragon Router
Cloud):
...
| Variable                               | Purpose                                 |
| -------------------------------------- | --------------------------------------- |
| `DRAGONROUTER_TRANSLATION_API_URL`     | OpenAI-compatible base URL, e.g. `…/v1` |
| `DRAGONROUTER_TRANSLATION_API_KEY`     | bearer token (kept out of logs)         |
| `DRAGONROUTER_TRANSLATION_MODEL`       | model id, e.g. `cx/gpt-5.4-mini`        |
...
- **Chunked batching**: Joins multiple strings with `__DRAGONROUTER_I18N_SEPARATOR__` delimiters to minimize API calls (max 1800 chars per request)
...
**Secondary translator** — uses any OpenAI-compatible LLM API (including Dragon Router itself) to translate existing `docs/i18n/` markdown files. Best for polishing or re-translating docs with better quality than Google Translate.
...
The `dragonrouter` CLI has its own i18n layer separate from the Next.js dashboard.
...
| -------- | --------------------------- | --------------------------------------------- |
| 1        | `--lang` flag               | `dragonrouter --lang de status`               |
| 2        | `DRAGONROUTER_LANG` env var | `DRAGONROUTER_LANG=ja dragonrouter providers` |
...
```bash
# Set language and save to ~/.dragonrouter/.env (persists across sessions)
dragonrouter config lang set pt-BR

# View current language
dragonrouter config lang get

# List all 42 available languages
dragonrouter config lang list

# JSON output
dragonrouter config lang list --output json
```

The saved preference is written atomically to `~/.dragonrouter/.env` and is loaded by the
...
dragonrouter --lang de providers list
```
