---
title: "Remote Mode — Drive a remote Dragon Router from your laptop"
version: 3.8.40
...
Run the `dragonrouter` CLI on your laptop while Dragon Router itself runs somewhere else
(a VPS, a home server, another machine on your Tailnet). You log in once with
`dragonrouter connect`, and from then on **every** CLI command targets that remote
server — same commands, same output, just executed against the remote.

There is no second tool to install: remote mode is the regular `dragonrouter` CLI
plus scoped **access tokens**.

```bash
npm install -g dragonrouter                 # the normal CLI
dragonrouter connect 192.168.0.15           # log in (password → scoped token)
dragonrouter models list                    # ← now lists the REMOTE server's models
dragonrouter configure codex                # ← writes a local Codex profile from the remote catalog
```
...
```
your laptop                              remote Dragon Router (VPS)
┌────────────────────┐                   ┌───────────────────────────────┐
│ dragonrouter CLI      │  POST /api/cli/connect  (password → token)         │
...
- **Contexts** store one server each (`~/.dragonrouter/config.json`, `chmod 600`).
  `dragonrouter contexts use <name>` switches the active server; `default` is local.
...
```bash
dragonrouter connect 192.168.0.15
...
```bash
dragonrouter connect 192.168.0.15 --scope write
```

Options: `--port <p>` (when the host has none), `--name <ctx>` (context name),
`--scope read|write|admin`. A full URL is honoured as-is:
`dragonrouter connect https://omni.example.com`.

### With a pre-generated token

Generate a scoped token in the dashboard (or with `dragonrouter tokens create`) and
paste it — no password needed:

```bash
dragonrouter connect 192.168.0.15 --key oma_live_xxxxxxxx
```
...
that uses the bundled Antigravity desktop client, not just Dragon Router.)

There are two supported ways to connect Antigravity to a remote Dragon Router.
...
npx dragonrouter login antigravity
...
#
#   dragonrouter-cred-v1.eyJ2IjoxLCJ...
```

Then, in the **remote** dashboard: **Providers → Antigravity → Connect**, and
paste the `dragonrouter-cred-v1.…` blob into the **Step 2** field (it accepts either
a callback URL or a credential blob). Dragon Router decodes it, runs the Cloud Code
...
```bash
dragonrouter tokens create --name "laptop" --scope write [--expires 30]
#   ↳ prints the secret ONCE — copy it now
dragonrouter tokens list                 # masked: id, name, scope, prefix, status, expiry
dragonrouter tokens revoke <id|prefix>   # revoke immediately
dragonrouter tokens scopes               # explain the three scopes
```
...
`dragonrouter configure` reads the **active server's** live model catalog and writes
a config on **your** machine.

```bash
dragonrouter configure codex
...
# non-interactive
dragonrouter configure codex --provider glm --model glm/glm-5.2 --name glm52
```

The written profile references the inference key by env var
(`DRAGONROUTER_API_KEY`) — the secret is never written to disk. For the one-time
base Codex setup (the `[model_providers.dragonrouter]` block), see
...
| ----------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Codex       | `dragonrouter setup-codex`    | `~/.codex/<name>.config.toml` profiles (per model)                                                                                                                      |
| Claude Code | `dragonrouter setup-claude`   | `~/.claude/profiles/<name>/settings.json` (per model)                                                                                                                   |
| OpenCode    | `dragonrouter setup-opencode` | `~/.config/opencode/opencode.json` — the `dragonrouter` openai-compatible provider with every catalog model (run `opencode -m dragonrouter/<model>`)                    |
| Cline       | `dragonrouter setup-cline`    | `~/.cline/data/{globalState,secrets}.json` (CLI mode) + prints the VS Code extension settings to paste (OpenAI-compatible, Base URL **without** `/v1`)                  |
| Kilo Code   | `dragonrouter setup-kilo`     | `~/.local/share/kilo/auth.json` (CLI) + VS Code `kilocode.*` settings — OpenAI-compatible, Base URL **with** `/v1`                                                      |
| Continue    | `dragonrouter setup-continue` | `~/.continue/config.yaml` (VS Code/JetBrains + `cn` CLI) — `provider: openai`, `apiBase` **with** `/v1`, key via `${{ secrets.DRAGONROUTER_API_KEY }}`                  |
| Cursor      | `dragonrouter setup-cursor`   | prints the in-app steps (Settings → Models → Override OpenAI Base URL **with** `/v1` + key + model). Cursor config is opaque SQLite — chat panel only                   |
| Roo Code    | `dragonrouter setup-roo`      | writes a Roo import JSON (`~/.dragonrouter/roo-settings.json`) + sets `roo-cline.autoImportSettingsPath` + prints UI steps (OpenAI-compatible, Base URL **with** `/v1`) |
| Crush       | `dragonrouter setup-crush`    | `~/.config/crush/crush.json` — `openai-compat` provider, `base_url` **with** `/v1`, key via `$DRAGONROUTER_API_KEY`                                                     |
| Goose       | `dragonrouter setup-goose`    | `~/.config/goose/config.yaml` (`GOOSE_PROVIDER=openai` + `OPENAI_HOST` **without** `/v1` + `GOOSE_MODEL`) + env recipe                                                  |
| Qwen Code   | `dragonrouter setup-qwen`     | `~/.qwen/settings.json` — openai `modelProvider`, `baseUrl` **with** `/v1`, key via `envKey` (DRAGONROUTER_API_KEY)                                                     |
| Aider       | `dragonrouter setup-aider`    | `~/.aider.conf.yml` (`openai-api-base` **without** `/v1` + `model: openai/<id>`) + env recipe (`aider --message --yes`)                                                 |
...
dragonrouter setup-opencode --only glm,kimi        # keep only matching models
opencode -m dragonrouter/glm/glm-5.2 "..."          # export DRAGONROUTER_API_KEY first
```

> OpenCode also has a richer **plugin** integration: `dragonrouter setup opencode`
> (now remote-aware via `--remote`) installs `@dragonrouter/opencode-plugin`.
> `setup-opencode` is the lightweight openai-compatible alternative. The API key
> is referenced via `{env:DRAGONROUTER_API_KEY}` — never written to disk.

---

## Managing contexts (switch between servers)

A **context** is a saved server (baseUrl + credential + scope). `dragonrouter connect`
creates one and makes it active; from then on every command targets it. Manage and
switch between them with `dragonrouter contexts`:

```bash
dragonrouter contexts list            # all contexts; the active one is marked ●
dragonrouter contexts current         # the active server, auth status, scope
```

```text
  | Name    | Base URL                  | Auth  | Scope | Description
● | vps     | http://100.67.86.91:20128 | token | admin | Remote Dragon Router (…)
...
```bash
dragonrouter contexts use vps         # → all commands now hit the remote VPS
dragonrouter tokens list              #   (runs against the VPS)

dragonrouter contexts use default     # → back to localhost
dragonrouter tokens list              #   (runs against the local server)
```

**Add a context manually** (instead of `connect`), inspect, or rename:

```bash
dragonrouter contexts add staging --url https://staging.example.com:20128 \
  --access-token oma_live_xxxx --scope write --description "staging box"
dragonrouter contexts show staging    # full details for one context
dragonrouter contexts rename staging stg
```
...
```bash
dragonrouter contexts remove stg --yes
```
...
> revoke the token on the server with `dragonrouter tokens revoke <id>` to actually
> kill access.
...
```bash
dragonrouter contexts export --out contexts.json     # default: stdout
dragonrouter contexts import contexts.json            # overwrite; --merge to keep existing
```
...
dragonrouter connect 192.168.0.15                 # or: --key oma_live_xxxx  (no password)
dragonrouter contexts current                     # shows the remote server + scope

# 2. Use it — management commands now run against the remote
dragonrouter tokens create --name laptop --scope read   # mint a narrower token
dragonrouter tokens list                                 # masked list, from the remote

# 3. Switch back and forth
dragonrouter contexts use default                 # → local
dragonrouter contexts use 192-168-0-15            # → remote again (name from `contexts list`)
...
dragonrouter tokens revoke <id|prefix>            # kills access on the server
dragonrouter contexts remove 192-168-0-15 --yes   # drop the local context (even if active → falls back to default), no prompt
```
...
- `dragonrouter connect` reuses the login brute-force lockout + audit logging.
...
- The local context file is `~/.dragonrouter/config.json` (`chmod 600`); tokens are
