...
Dragon Router exposes three different agent-related surfaces. They look similar at first glance but solve different problems. Use this page to pick the right one.
...
**Dragon Router endpoint:** `POST /a2a` (JSON-RPC 2.0)
**Agent Card:** `GET /.well-known/agent.json`

### When to use

- Building a multi-agent system where Dragon Router is one of the peers
- Exposing Dragon Router's routing intelligence (smart-routing, quota-management, etc.) to agents in frameworks like Google ADK or generic agent meshes
- Wrapping Dragon Router behind a standard discovery + invocation surface
...
**Dragon Router endpoint:** `GET /api/acp/agents`
**Source:** `src/lib/acp/{index,manager,registry}.ts`

### What it is

ACP is Dragon Router's **local CLI agent inventory**. It detects which coding CLIs are installed on the host (Cursor, Cline, Claude Code, Codex CLI, Continue, etc.), resolves their versions, and surfaces them to the dashboard so the user can wire each CLI to point at Dragon Router.
...
- Dashboard "CLI Tools" page lists what's installed and helps you point each at Dragon Router
- Custom agents let power users register internal/proprietary CLIs that Dragon Router doesn't know about by default
- Detection result fuels the `cli-tools` fingerprint matrix

### When NOT to use ACP

- ACP doesn't _run_ tasks. It only detects + configures CLIs. To actually invoke a CLI, you launch it yourself with the env vars Dragon Router provides (`OPENAI_BASE_URL`, `OPENAI_API_KEY`, etc.).

## 3. Cloud Agents

**Dragon Router endpoints:** `/api/v1/agents/tasks/*` (lifecycle) + `/api/cloud/*` (plumbing)
**Source:** `src/lib/cloudAgent/`

### What it is

A uniform interface over third-party cloud coding agents. You submit a prompt + repo URL, Dragon Router dispatches to the right cloud agent, polls status, returns results.
...
### Discover Dragon Router's A2A capabilities
...
### Call Dragon Router as an A2A agent
