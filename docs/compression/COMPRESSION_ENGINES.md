...
Dragon Router compression is built around engine contracts. A mode can run one engine directly
...
with a `22-87%` range, and a `~46%` input-compression tool. Dragon Router uses the Caveman input-side
...
Dragon Router install; when the optional stack is absent, LLMLingua still fail-opens. Only the three
...
- **Global npm (`npm i -g dragonrouter`) / standalone** — run the install command above inside
...
| `dragonrouter_list_compression_combos` | `read:compression`  | List compression combos          |
| `dragonrouter_compression_combo_stats` | `read:compression`  | Read combo/engine analytics      |
