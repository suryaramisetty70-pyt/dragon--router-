...
> **TL;DR**: Dragon Router's compression engine is **pluggable** — you can register custom engines, ship language packs for new languages, and compose stacked pipelines. This guide shows how.
...
````ts
import type { CompressionEngine } from "dragonrouter/compression/engines/types";
import { registerCompressionEngine } from "dragonrouter/compression/engines/registry";
...
```
~/.dragonrouter/compression/engines/my-engine.ts    # User-level
...
} from "@dragonrouter/open-sse/services/compression/engines/registry";
...
Caveman-style compression uses **language-specific rule packs** to handle fillers, hedging, and verbose patterns in each natural language. Dragon Router ships with **6 language packs**: `en`, `es`, `fr`, `de`, `ja`, `pt-BR`.
...
```ts
import { loadRulePack } from "dragonrouter/compression/ruleLoader";
...
```
~/.dragonrouter/compression/rules/hi/filler.json  # User-level
...
Dragon Router selects **ONE mode per request** based on configuration, auto-trigger thresholds, and combo overrides.
...
`@dragonrouter/open-sse/services/compression/strategySelector`):

```ts
import { applyStackedCompression } from "@dragonrouter/open-sse/services/compression/strategySelector";
...
Dragon Router's compression engines credit several upstream projects in the README
...
pack, how does that reach Dragon Router?** This section is the authoritative answer.
...
rule/filter in Dragon Router's own format** (see
...
Dragon Router's own `smartcrusher` / `toon` / `tabular` layers. There is no live
...
1. **Don't vendor** — re-express the upstream rule/filter in Dragon Router's format.
