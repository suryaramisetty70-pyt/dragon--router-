...
Dragon Router CLI commands authenticate against the local management API using a
`HMAC-SHA256(machine-id, salt)` token sent via the `x-dragonrouter-cli-token`
request header.

This allows CLI subcommands (`dragonrouter status`, `dragonrouter providers`, etc.)
...
3. The CLI sends the token as `x-dragonrouter-cli-token` on every request to
...
Set `DRAGONROUTER_CLI_SALT` to rotate the derived token without code changes.
...
export DRAGONROUTER_CLI_SALT="my-secret-salt-2026"

# Verify new token is in use
dragonrouter status
```

Default salt: `dragonrouter-cli-auth-v1`
...
**Opt-out:** set `DRAGONROUTER_DISABLE_CLI_TOKEN=true` (env or `.env`) to disable the CLI
