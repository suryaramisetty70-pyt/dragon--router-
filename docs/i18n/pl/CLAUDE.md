...
**Dragon Router** — zjednoczony proxy/router AI. Jeden punkt końcowy, 160+ dostawców LLM, automatyczne przełączanie.
...
Dragon Router ma trzy powiązane, ale odrębne mechanizmy tymczasowej awarii. Zachowaj ich zakres oddzielnie podczas debugowania zachowania routingu. Zobacz
...
- **Importy**: zewnętrzne → wewnętrzne (`@/`, `@dragonrouter/open-sse`) → względne
...
- **Alias ścieżek**: `@/*` → `src/`, `@dragonrouter/open-sse` → `open-sse/`, `@dragonrouter/open-sse/*` → `open-sse/*`
- **Domyślny port**: 20128 (API + dashboard na tym samym porcie)
- **Katalog danych**: zmienna środowiskowa `DATA_DIR`, domyślnie `~/.dragonrouter/`
