# Arsitektur Dragon Router (Bahasa Indonesia)
...
Dragon Router adalah gateway routing AI lokal dan dasbor yang dibangun di atas Next.js.
...
- File: `${DATA_DIR}/storage.sqlite` (atau `$XDG_CONFIG_HOME/dragonrouter/storage.sqlite` jika diatur, jika tidak `~/.dragonrouter/storage.sqlite`)
...
- respons klien akhir yang dikembalikan oleh Dragon Router; tanggapan yang dialirkan disimpan dalam bentuk ringkasan ringkas yang sama
...
1. `usageDb` dan `localDb` berbagi kebijakan direktori dasar yang sama (`DATA_DIR` -> `XDG_CONFIG_HOME/dragonrouter` -> `~/.dragonrouter`) dengan migrasi file lama.
...
5. Direktori `open-sse/` diterbitkan sebagai `@dragonrouter/open-sse` **paket ruang kerja npm**. Kode sumber mengimpornya melalui `@dragonrouter/open-sse/...` (diselesaikan oleh Next.js `transpilePackages`). Jalur file dalam dokumen ini masih menggunakan nama direktori `open-sse/` untuk konsistensi.
...
- Bangun gambar Docker: `docker build -t dragonrouter .`
