...
<img src="../../screenshots/MainDragon Router.png" alt="Dragon Router 控制台" width="820"/>
...
**约 1.6B 可统计免费 Token / 月** — 计入注册奖励后，首月最高可达 **~2.1B** — 聚合各家免费层配额，外加一众永久免费、不限量的服务商；再叠加上述压缩引擎，每一枚 Token 都物超所值。([统计方法 →](../../reference/FREE_TIERS.md#tldr--how-much-free-inference-does-dragonrouter-actually-aggregate))
...
[![Telegram](https://img.shields.io/badge/Telegram-26A5E4?style=for-the-badge&logo=telegram&logoColor=white)](https://t.me/dragonrouterOficial)
...
**疑难解答、服务商攻略、路线图与支持 → [Discord](https://discord.gg/EkzRkpzKYt) · [Telegram](https://t.me/dragonrouterOficial) · WhatsApp [🌍 全球](https://chat.whatsapp.com/JI7cDQ1GyaiDHhVBpLxf8b?mode=gi_t) / [🇧🇷 巴西](https://chat.whatsapp.com/BTGJXIyjeNIIgExvTMGGhI)**

<br/>

<a href="https://trendshift.io/repositories/23589" target="_blank"><img src="https://trendshift.io/api/badge/repositories/23589" alt="diegosouzapw%2FDragon Router | Trendshift" style="width: 250px; height: 55px;" width="250" height="55"/></a>

[![npm](https://img.shields.io/npm/v/dragonrouter?logo=npm&style=flat-square)](https://www.npmjs.com/package/dragonrouter)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg?style=flat-square)](../../LICENSE)
[![Node](https://img.shields.io/badge/node-%E2%89%A522.0.0-brightgreen?style=flat-square)](../../package.json)
[![Stars](https://img.shields.io/github/stars/diegosouzapw/Dragon Router?style=social)](https://github.com/diegosouzapw/Dragon Router)

<div align="center">

[![npm version](https://img.shields.io/npm/v/dragonrouter?color=cb3837&logo=npm)](https://www.npmjs.com/package/dragonrouter)
![NPM Monthly](https://img.shields.io/npm/dm/dragonrouter?label=npm/month&color=cb3837&logo=npm)
...
![Docker Pulls](https://img.shields.io/docker/pulls/diegosouzapw/dragonrouter?label=docker%20pulls&logo=docker&color=2496ED)
![Electron Downloads](https://img.shields.io/github/downloads/diegosouzapw/dragonrouter/total?style=flat&label=electron%20downloads&logo=electron&color=47848F)
...
[**🚀 快速开始**](#-quick-start) • [**🎯 Combo**](#-combos--the-flagship) • [**🌐 服务商**](#-231-ai-providers--50-free) • [**🔌 CLI 与 MCP**](#-full-cli--a2a--mcp) • [**🗜️ 压缩**](#%EF%B8%8F-save-1595-tokens--automatically) • [**🌍 官网**](https://dragonrouter.online)

[💥 我们的承诺](#-the-promise) • [🤔 为什么选择 Dragon Router](#-why-dragonrouter) • [🏆 核心优势](#-what-sets-dragonrouter-apart) • [🤖 兼容的编程工具](#-compatible-clis--coding-agents) • [🖥️ 运行平台](#%EF%B8%8F-where-dragonrouter-runs--anywhere) • [🔒 隐私优先](#-private--local-first) • [🎬 实机演示](#-dragonrouter-in-action) • [📚 探索更多](#-explore-more) • [📧 支持](#-support--community)
...
> 手动凑各家免费额度有多痛苦 — 数十套 SDK、数十个速率限制，根本搞不清到底还剩多少。Dragon Router 将 **40+ 服务商池 / 500+ 模型**的**可核实**免费层聚合为一个真实的统一数字，并在控制台实时展示 (`/dashboard/free-tiers`)。
>
...
> 一个端点。**236 家服务商。** 编码不止步 — 让 Dragon Router 帮你选出最便宜且可用的那个。
...
> **Combo** 是 Dragon Router **自动**路由的模型接力链路。配额耗尽、服务商宕机或成本飙升 — Combo 自动滑向下一个模型，无声无息。**正是它让 Dragon Router 坚不可摧。** 🛡️

### ⚡ 零配置 — 只需设为 `auto`

无需预先配置 Combo。将模型 ID 设为 `auto`（或其变体），Dragon Router 会基于你已连接的服务商实时评分，自动构建虚拟 Combo：
...
# 🏆 Dragon Router 何以脱颖而出

</div>

| 功能 | Dragon Router | 其他路由方案 |
|---|---|---|
| 🌐 服务商数量 | **231** | 20–100 |
...
<sub>📊 与 LiteLLM、OpenRouter、Portkey 的详细对比 → [`docs/comparison/DRAGONROUTER_VS_ALTERNATIVES.md`](../../comparison/DRAGONROUTER_VS_ALTERNATIVES.md)</sub>
...
- **🤖 一键 CLI/Agent 配置** — 专用 `setup-*` 命令为各编程工具一键配置 Dragon Router 路由（Claude Code、Codex、Cline、Continue、Cursor、Roo Code、Kilo Code、Crush、Goose、Qwen Code、Aider、OpenCode）；`dragonrouter launch` / `dragonrouter launch-codex` 为零配置启动器。→ [CLI 集成](../../guides/CLI-INTEGRATIONS.md)
- **🛰️ 远程模式** — 通过授权范围 Token 从任意机器操控远程 Dragon Router（`dragonrouter connect` / `dragonrouter contexts` / `dragonrouter tokens`）；另附 `dragonrouter login antigravity` 辅助命令，在你的本机运行 Google "native/desktop" OAuth 后将凭证 blob 粘贴至远程/VPS 安装实例（因远程环境无法接收 loopback 回调）。→ [远程模式](../../guides/REMOTE-MODE.md)
...
- **⚡ 本地性能与基础设施** — 一键本地 Redis 启动器（`dragonrouter redis up`，含控制台 Redis 面板）、一键 **Cloudflare Workers** 和 **Deno Deploy** 中继部署器（接入代理池），以及可选 Bifrost Go 边车将最热中继路径卸载至 Go 侧（`BIFROST_BASE_URL`，超时自动回退 TypeScript 路径）— 现支持中继后端选择器（`DRAGONROUTER_RELAY_BACKEND=ts|bifrost|auto`），`/v1/relay` 端点保持对外稳定接口的同时内部自动择取最快后端。→ [环境配置](../../reference/ENVIRONMENT.md)
...
<sub>📖 16+ 款工具的逐项配置指南 → [`docs/reference/CLI-TOOLS.md`](../../reference/CLI-TOOLS.md) · 🧩 OpenCode 插件 → [`@dragonrouter/opencode-provider`](https://www.npmjs.com/package/@dragonrouter/opencode-provider)</sub>
...
| 平台 | 安装方式 | 亮点 |
|---|---|---|
| 📦 **npm（全局）** | `npm install -g dragonrouter` | 一行命令，任意 OS |
| 🐳 **Docker** | `docker run … diegosouzapw/dragonrouter` | 多架构 **AMD64 + ARM64** |
| 🖥️ **桌面（Electron）** | `npm run electron:build` | 原生窗口 + 系统托盘 — **Windows / macOS / Linux** |
| 💪 **ARM** | 原生 `arm64` | 树莓派、ARM 服务器、Apple Silicon |
| 📱 **Android（Termux）** | `pkg install nodejs && npx -y dragonrouter` | **在手机上** 7×24 运行，无需 Root |
| 📲 **PWA** | "添加到主屏幕" | 全屏、离线、可从浏览器安装 |
| 🧩 **OpenCode 插件** | `@dragonrouter/opencode-provider` | 原生 OpenCode 集成 |
...
> 你的密钥、你的机器、你的数据。Dragon Router 是**本地代理** — 绝不会向外回传。

- 🏠 **100% 运行在本地硬件上** — npm、Docker、桌面端或你的手机。请求链路中不存在任何 Dragon Router 云端节点。
...
> Dragon Router 不只是一台服务器 — 它是拥有 **60+ 命令**的**全功能命令行驾驶舱**，外加开放的代理协议，让 AI 代理**自主**操控 Dragon Router。

### ⌨️ 真正的 CLI（不止 `start`）

```bash
dragonrouter               # 启动网关 + 控制台（端口 20128）
dragonrouter chat          # 交互式 TUI 聊天客户端（斜杠命令：/model /combo /skill /memory）
dragonrouter setup         # 引导式首次设置向导
dragonrouter doctor        # 诊断服务商、端口、原生依赖
```

### 🛰️ 远程模式 — CLI 在本地，Dragon Router 在远端的 VPS

Dragon Router 跑在服务器上？用**同一套 CLI** 从笔记本远程操控。登录一次，绑定授权范围 Token；后续所有命令自动指向远端。

```bash
dragonrouter connect 192.168.0.15            # 密码 → 范围 Token，保存为上下文
dragonrouter models list                     # ← 在远端服务器上执行
dragonrouter configure codex                 # ← 选择远端模型，写入本地 Codex 配置文件
dragonrouter tokens create --name ci --scope read   # 为其他机器签发更窄范围的 Token
dragonrouter contexts use default            # ← 切回本机服务器
```
...
### 🤝 接入 AI 代理 — 让代理自主操控 Dragon Router

通过 **MCP** 或 **A2A** 协议暴露 Dragon Router，任何智能代理都能获得网关的完整控制权 — 路由、服务商、Combo、缓存、压缩、记忆 — 全自主运行。

| 协议 | 端点 | 用途 |
|---|---|---|
| 🧰 **MCP（stdio）** | `dragonrouter --mcp` | 接入 Claude Desktop、Cursor 等各种 MCP 客户端 |
...
```bash
# 通过 MCP 将 Dragon Router 完整工具集赋予 Claude Code：
...
> **Token 够用就好，何必铺张浪费？** 每个请求**透明地**通过 Dragon Router 压缩流水线 — 客户端无需任何改动。现已升级为 **9 大可组合引擎**的级联体系，按 Combo 自由排列组合 — 凝聚了 [RTK](https://github.com/rtk-ai/rtk)、[Caveman](https://github.com/JuliusBrussee/caveman)（⭐ 51K+）、[LLMLingua-2](https://github.com/microsoft/LLMLingua) 和 [Troglodita](https://github.com/leninejunior/troglodita)（PT-BR）的技术精华。
...
```
Client (10,000 tok) ──▶ Dragon Router Compression (9 engines) ──▶ Provider (~1,080 tok, 节省高达 95%)
```
...
- **🎛️ 压缩决策的优先链路** _（从高到低）_ — 逐请求 `x-dragonrouter-compression` 头 › Combo 覆写 › 活动命名配置 › 自适应/自动触发 › 面板默认 › 关闭。最终采用的压缩方案会通过 `X-Dragon Router-Compression: <mode>; source=<source>` 响应头回显。
...
你应该能看到已连接模型的列表。🎉 至此大功告成 — 开始编码，Dragon Router 自动路由、自动容灾。

如果你的客户端无法发送自定义请求头，Dragon Router 也提供 Token 化兼容别名：
...
```bash
docker run -d --name dragonrouter --restart unless-stopped --stop-timeout 40 \
  -p 20128:20128 -v dragonrouter-data:/app/data diegosouzapw/dragonrouter:latest
```
...
```bash
pnpm add -g dragonrouter@latest --allow-build=better-sqlite3 --allow-build=@swc/core && dragonrouter
```

**🐧 Arch Linux（AUR）**

```bash
yay -S dragonrouter-bin && systemctl --user enable --now dragonrouter.service
```
...
```bash
# 1. 构建镜像
podman build --target runner-base -t dragonrouter:base .
...
> 🎬 **制作了关于 Dragon Router 的视频？** 通过链接创建 [issue](https://github.com/diegosouzapw/Dragon Router/issues/new) 或 [discussion](https://github.com/diegosouzapw/Dragon Router/discussions) — 我们将在本节予以展示。
...
> 💡 控制台上的"费用"是**节省追踪器**，而非账单 — Dragon Router 从不向你收费。显示"$290 总费用"意味着你使用免费模型**省下了 $290**。
...
🇷🇺 🇨🇳 🇮🇷 🇨🇺 🇹🇷 身处受限地区？Dragon Router 的**三级代理体系**（全局 / 按服务商 / 按连接）代理 API 请求、OAuth 流程、连通性测试、Token 刷新和模型同步。
...
| `DATA_DIR` | `~/.dragonrouter` | 数据库与配置存储路径 |

**Dragon Router 会向我收费吗？** 不会 — 它是运行在你本机的免费开源软件。你只直接向付费服务商付款。Dragon Router 不含任何计费系统。
...
- 🌍 **官网**：[dragonrouter.online](https://dragonrouter.online)
- 🐙 **GitHub**：[github.com/diegosouzapw/Dragon Router](https://github.com/diegosouzapw/Dragon Router)
- 🐛 **Issues**：[报告 Bug](https://github.com/diegosouzapw/Dragon Router/issues)（请附上 `npm run system-info` 的输出结果）
...
- **官网**：[dragonrouter.online](https://dragonrouter.online)
- **npm 包**：[npmjs.com/package/dragonrouter](https://www.npmjs.com/package/dragonrouter)
- **Docker**：[hub.docker.com/r/diegosouzapw/dragonrouter](https://hub.docker.com/r/diegosouzapw/dragonrouter)
...
| [远程模式](../../guides/REMOTE-MODE.md) | 通过授权范围 Token 从笔记本 CLI 操控远端 Dragon Router（VPS） |
| [Claude Code 配置](../../guides/CLAUDE-CODE-CONFIGURATION.md) | 使用 `launch` + 按模型配置文件将 Claude Code 指向 Dragon Router（本地/远程） |
...
| [Termux 指南](../../guides/TERMUX_GUIDE.md) | 通过 Termux 在 Android 上运行 Dragon Router |
...
> Dragon Router 由充满热情的开源社区共同塑造。以下同仁做出了卓越贡献，直接影响着项目的质量、稳定性与影响力。**衷心感谢。**
...
> 🙏 这些贡献者的功能、Bug 修复和基础设施改进，是 Dragon Router 可靠且功能丰富的**核心支柱**。每一个 Pull Request、每一个测试用例、每一个 i18n 翻译文件都意义重大。开源正是由他们这样的人建造的。
...
Dragon Router 是站在巨人肩膀上的作品。它始于 **[9router](https://github.com/decolua/9router)** 的一个 Fork 以及 Go 项目 **[CLIProxyAPI](https://github.com/router-for-me/CLIProxyAPI)** 的 TypeScript 移植 — 自此，以下每个子系统均受惠于先行者的开源成果。每一个项目都在 Dragon Router 中留下了具体印记。这是我们对所有项目的由衷感谢。🙏

> ⭐ 星标数为 2026 年 6 月数据 — 请给这些项目点颗星。

### 🧬 渊源与网关

| 项目 | ⭐ | 对 Dragon Router 的启发 |
|---|---|---|
...
Dragon Router 是免费开源项目，在公开环境中持续构建与维护。如果它帮你节省了时间或金钱，请考虑以以下方式支持开发：

- ⭐ **为本仓库加颗 Star** — 这确确实实能帮我们提升可见度
- 💖 **[GitHub Sponsors](https://github.com/sponsors/diegosouzapw)** — 资助持续维护和新服务商接入
- 🐛 **在 [Discussions](https://github.com/diegosouzapw/Dragon Router/discussions) 中反馈 Bug 和分享意见**
...
**[⬆ 返回顶部](#-dragonrouter)** · 用 ❤️ 为开源 AI 社区构建。

<sub>Dragon Router v3.8.24 · Node ≥22.0.0 · MIT License · <a href="https://dragonrouter.online">dragonrouter.online</a></sub>
