---
title: "CLI 工具 — Dragon Router"
version: 3.8.40
lastUpdated: 2026-06-28
---

# CLI 工具 — Dragon Router
...
Claude / Codex / OpenCode / Cline / KiloCode / Continue / Hermes Agent / Goose / ...
    ...
    http://YOUR_SERVER:20128/v1
           │
           ▼  （Dragon Router 路由到合适的服务商）
    Anthropic / OpenAI / Gemini / DeepSeek / Groq / Mistral / ...

ACP Agents（反向启动流程）：
    客户端请求 → Dragon Router → 通过 stdio/ACP 启动 CLI → 响应
```
...
无需手动编写每个工具的配置。Dragon Router 为每个支持的 CLI 提供对应的 `setup-*` 命令，该命令从运行中的 Dragon Router（本地或远程）读取**实时**模型目录并将工具的配置写入你的机器：
...
每个命令接受 `--remote <url> --api-key <key>`（针对远程 Dragon Router 配置本地工具）、`--dry-run`（预览不写入）和 `--port`。不支持模型自动发现的工具（Cline、Kilo、Roo、Goose、Qwen、Aider、Gemini）接受 `--model <id>`（以及用于非交互式运行的 `--yes`）。启动器 `dragonrouter launch`（Claude Code）和 `dragonrouter launch-codex`（Codex）在注入正确的环境变量后启动 CLI，不写入任何配置。
...
此页面（从 `/dashboard/agents` 重命名而来）显示 Dragon Router 可以通过 stdio/ACP 协议**启动**为后端执行引擎的 CLI。目录在 `src/lib/acp/registry.ts` 中单独维护，**不同于** `CLI_TOOLS`。
...
> 使用 `opencode run "your prompt" --model dragonrouter/claude-sonnet-4-5-thinking --variant high`
> 发送 thinking 变体。
...
或使用 Dragon Router dashboard → **CLI Tools → Cline → 应用配置**。
...
或使用 Dragon Router dashboard → **CLI Tools → KiloCode → 应用配置**。
...
当 VS Code Insiders 配置了自定义端点模型，且你希望 Dragon Router 在不使用自定义请求头字段的情况下工作时使用。
...
**使用 Token 化 Dragon Router 别名的示例：**
...
- 将 `sk-your-dragonrouter-key` 替换为在 Dragon Router 中创建的 API Key。
...
# CLI 使用自己的认证 — Kiro CLI 本身不需要 Dragon Router 作为后端。
# 将 kiro-cli 与 Dragon Router 配合用于其他工具。
kiro-cli status
```

对于 **Kiro IDE** 桌面应用，使用 Dragon Router 通过 `/dashboard/cli-tools → Kiro` 暴露的 MITM 端点。
...
> Qwen OAuth 免费层已于 2026-04-15 停用。改为使用 Dragon Router 搭配
...
## 10. Dragon Router 内置 CLI

`dragonrouter` 二进制文件提供服务端生命周期、设置、诊断和服务商管理的命令。入口点：`bin/dragonrouter.mjs`。

```bash
dragonrouter                              # 启动服务器（默认端口 20128）
dragonrouter setup                        # 交互式设置向导
dragonrouter doctor                       # 检查配置、数据库、端口、运行时
dragonrouter providers list               # 已配置的服务商连接
dragonrouter providers test-all           # 测试所有活跃连接
dragonrouter reset-password               # 重置管理员密码
dragonrouter logs                         # 流式输出请求日志
dragonrouter health                       # 详细健康状态（熔断器、缓存、内存）
dragonrouter --version                    # 输出版本号
dragonrouter --help                       # 显示所有命令
```
...
```bash
dragonrouter setup                        # 交互式设置向导
dragonrouter setup --non-interactive      # CI/自动化模式（读取环境变量 + 标志）
dragonrouter setup --password '<value>'   # 直接设置管理员密码
...
（参见上述 `dragonrouter setup` 选项）。

### 诊断

```bash
dragonrouter doctor                       # 检查配置、数据库、端口、运行时、内存、存活状态
dragonrouter doctor --json                # 机器可读的 JSON
dragonrouter doctor --no-liveness         # 跳过 HTTP 健康探测
dragonrouter doctor --host 0.0.0.0        # 覆盖存活探测的主机
dragonrouter doctor --liveness-url <url>  # 完全覆盖健康端点 URL
```
...
```bash
dragonrouter providers available                       # Dragon Router 服务商目录
dragonrouter providers available --search openai       # 按 id/name/alias/category 过滤目录
dragonrouter providers available --category api-key    # 按分类过滤（api-key、oauth、free 等）
dragonrouter providers available --json                # 机器可读的 JSON

dragonrouter providers list                            # 已配置的服务商连接
dragonrouter providers list --json

dragonrouter providers test <id|name>                  # 测试一个已配置的连接
dragonrouter providers test-all                        # 测试所有活跃连接
dragonrouter providers validate                        # 仅本地结构校验
```

> `providers available` 读取 Dragon Router 目录；`providers list/test/test-all/validate`
> 直接读取本地 SQLite 数据库，不需要服务器运行。

### 恢复与重置

```bash
dragonrouter reset-password                # 重置管理员密码（旧别名仍然可用）
dragonrouter reset-encrypted-columns       # 显示加密凭证重置的警告 + 干运行
dragonrouter reset-encrypted-columns --force  # 实际在 SQLite 中将加密凭证设为 null
```

### 其他子命令

以下命令假定 Dragon Router 服务器正在运行（另有说明除外）：

```bash
dragonrouter status                       # 全面的运行时状态
dragonrouter logs                         # 流式输出请求日志（--json、--search、--follow）
dragonrouter config show                  # 显示当前配置

dragonrouter provider list                # 列出可用服务商（providers list 的别名）
dragonrouter provider add                 # 将 Dragon Router 注册为某个工具的服务商
dragonrouter keys add | list | remove     # 管理 API Key
dragonrouter models [provider]            # 列出模型（--json、--search）
dragonrouter combo list | switch | create | delete

dragonrouter backup                       # 快照配置 + 数据库
dragonrouter restore                      # 从之前的快照恢复

dragonrouter health                       # 详细健康状态（熔断器、缓存、内存）
dragonrouter quota                        # 服务商配额用量
dragonrouter cache                        # 缓存状态
dragonrouter cache clear                  # 清除语义 + 签名缓存

dragonrouter mcp status | restart         # MCP 服务器状态 / 重启
dragonrouter a2a status | card            # A2A 服务器状态 / agent card

dragonrouter tunnel list | create | stop  # 管理隧道（cloudflare/tailscale/ngrok）
dragonrouter env show | get <k> | set <k> <v>  # 检查 / 设置环境变量（临时）

dragonrouter test                         # 服务商连通性冒烟测试
dragonrouter update                       # 检查更新
dragonrouter completion                   # 生成 shell 补全
```
