...
本指南介绍如何从系统中彻底移除 Dragon Router。

---

## 快速卸载（v3.6.2+）

Dragon Router 提供两个内置脚本用于干净移除：
...
此命令移除 Dragon Router 应用程序，但**保留** `~/.dragonrouter/` 中的数据库、配置、API Key 和服务商设置。如果你计划稍后重装并希望保留现有配置，请使用此方式。
...
- 打开 `设置 → 应用 → Dragon Router → 卸载`
- 或从安装目录运行 NSIS 卸载程序

**macOS：**

- 将 `Dragon Router.app` 从 `/Applications` 拖入废纸篓
- 删除数据：`rm -rf ~/Library/Application Support/dragonrouter`

**Linux：**

- 删除 AppImage 文件
- 删除数据：`rm -rf ~/.dragonrouter`
