...
**Dragon Router** — proxy/router AI terpadu. Satu endpoint, 160+ penyedia LLM, auto-fallback.
...
Dragon Router memiliki tiga mekanisme kegagalan sementara yang terkait tetapi berbeda. Jaga agar ruang lingkup mereka terpisah saat melakukan debugging perilaku pengalihan. Lihat diagram
...
- **Impor**: eksternal → internal (`@/`, `@dragonrouter/open-sse`) → relatif
...
- **Alias jalur**: `@/*` → `src/`, `@dragonrouter/open-sse` → `open-sse/`, `@dragonrouter/open-sse/*` → `open-sse/*`
- **Port default**: 20128 (API + dashboard di port yang sama)
- **Direktori data**: variabel lingkungan `DATA_DIR`, default ke `~/.dragonrouter/`
