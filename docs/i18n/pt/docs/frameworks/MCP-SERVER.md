# Dragon Router MCP Server Documentation (Português (Portugal))
...
| `dragonrouter_test_combo`             | Live-test all models in a combo via a real upstream request |
| `dragonrouter_get_provider_metrics`   | Detailed metrics for one provider                           |
| `dragonrouter_best_combo_for_task`    | Task-fitness recommendation with alternatives               |
| `dragonrouter_explain_route`          | Explain a past routing decision                             |
| `dragonrouter_get_session_snapshot`   | Full session state: costs, tokens, errors                   |
