...
**Dragon Router** — єдиний AI проксі/маршрутизатор. Один кінцевий пункт, 160+ постачальників LLM, автоматичне резервування.
...
Dragon Router має три пов'язані, але різні механізми тимчасових збоїв. Тримайте їх
...
- **Імпорти**: зовнішні → внутрішні (`@/`, `@dragonrouter/open-sse`) → відносні
...
- **Псевдоніми шляхів**: `@/*` → `src/`, `@dragonrouter/open-sse` → `open-sse/`, `@dragonrouter/open-sse/*` → `open-sse/*`
- **Порт за замовчуванням**: 20128 (API + панель управління на одному порту)
- **Директорія даних**: `DATA_DIR` змінна середовища, за замовчуванням `~/.dragonrouter/`
