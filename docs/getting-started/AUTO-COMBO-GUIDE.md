# Auto-Combo: Let Dragon Router Pick the Best AI for You

> **TL;DR**: Set your model to `auto` and Dragon Router automatically picks the best AI provider for each request. No configuration needed.

---

## What It Does

Instead of choosing a specific AI model (like GPT-4o or Claude), you can let Dragon Router **automatically pick the best one** for each request. It considers:
...
Dragon Router scores all your connected providers and picks the best one. If it fails, it automatically tries the next one.
...
**Step 2**: That's it! Dragon Router handles the rest.
...
When you send a request with `model: "auto"`, Dragon Router:
...
4. **Auto-recovers** — If it fails, Dragon Router tries the next one automatically
...
Dragon Router has **three layers of protection**:

### 1. Auto-Fallback

If the best provider fails, Dragon Router automatically tries the next one. You don't need to do anything.
...
If all providers fail, Dragon Router routes to stable free providers (like Kiro or Qoder) as a last resort.

---

## Multi-Account Support

If you have multiple accounts for the same provider (e.g., two OpenAI keys), Dragon Router treats each as a **separate candidate**. This means:
...
Dragon Router occasionally **explores** new providers to discover better options:
...
This helps Dragon Router learn which providers work best for your usage patterns.
...
Dragon Router automatically skips it and tries the next one. If a provider keeps failing, it's excluded temporarily (5-30 minutes). You don't need to do anything.

### "Can I see which provider was used?"

Check the response headers — Dragon Router includes the provider and model used in each response.
