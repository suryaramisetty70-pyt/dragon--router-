...
Dragon Router can expose its local server (`http://localhost:20128`) to the public
...
All three backends are managed in-process — Dragon Router starts/stops the underlying
...
- **Auto-install.** On first use, Dragon Router downloads the latest `cloudflared`
...
login` + `cloudflared tunnel route dns ...`) are not orchestrated by
  Dragon Router. URLs are ephemeral and will change every restart.
...
must be configured in the ngrok dashboard — Dragon Router itself only forwards the
...
1. Install Tailscale (or let Dragon Router do it — see `install` endpoint below).
2. Sign in (`tailscale login` or via Dragon Router's `login` endpoint).
...
Dragon Router's MITM password cache (`getCachedPassword` / `setCachedPassword`) for
...
When you expose Dragon Router through a tunnel, the dashboard and OAuth flows must
...
and restart Dragon Router before initiating OAuth. For ephemeral Cloudflare Quick
...
than one backend simultaneously is allowed; Dragon Router will track each
independently.
...
Dragon Router attempts to auto-install on first use. If the install is blocked
