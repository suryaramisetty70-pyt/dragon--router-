---
title: "Dragon Router Fly.io Deployment Guide"
version: 3.8.40
lastUpdated: 2026-06-28
---

# Dragon Router Fly.io Deployment Guide

This document describes the actual deployment process for Dragon Router on Fly.io, covering two scenarios:
...
This guide is based on a verified working configuration for the current project. The application name is `dragonrouter`.
...
- Access URL: `https://dragonrouter.fly.dev/`
...
- `app = 'dragonrouter'` determines which Fly application the deployment targets
...
- If you previously used a different name, such as `oroute`, do not confuse it with `dragonrouter`
...
If you changed the application name, replace `dragonrouter` with your chosen name.
...
These parameters have been used in actual deployments on the current `dragonrouter` application:
...
- `DRAGONROUTER_WS_BRIDGE_SECRET` (required in production — used for WebSocket bridge authentication)
...
1. **Set `NEXT_PUBLIC_BASE_URL` to your public HTTPS domain**
   ...
   If you are using a custom domain, replace it with the corresponding domain (e.g. `https://dragonrouter.yourdomain.com`).
...
- Intended for the current project `dragonrouter`
...
- `DRAGONROUTER_WS_BRIDGE_SECRET` is required in production; missing it will break the WebSocket bridge handshake

If you also want to set an initial password:

```powershell
flyctl secrets set INITIAL_PASSWORD=your-strong-password -a dragonrouter
```
...
- That you are viewing the `dragonrouter` application
...
If the current repository is a fork and you want to sync updates from the upstream `https://github.com/diegosouzapw/Dragon Router`, follow the workflow below.
...
- You are viewing a different application (e.g. `oroute` instead of `dragonrouter`)
