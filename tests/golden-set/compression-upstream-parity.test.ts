...
function dragonrouterCompress(text: string): string {
  return dragonrouterPromptCompression(text).text;
}

function dragonrouterPromptCompression(text: string): { text: string; fallbackApplied: boolean } {
...
describe("upstream Caveman parity benchmark", () => {
  it("matches core upstream shrink protections and savings direction", () => {
    for (const input of parityCases) {
      const ours = dragonrouterCompress(input);
      const theirs = upstream.compress(input).compressed;
      assert.ok(ours.length <= input.length, `Dragon Router did not reduce: ${input}`);
  ...
  it("stays within upstream token budget on representative prose", () => {
    ...
    assert.ok(
      ...
      `Expected Dragon Router within 20% of upstream shrink length. ours=${ours.length}, upstream=${theirs.length}`
    );
  });
  ...
  it("runs offline parity against upstream Caveman fixture files", () => {
    ...
    for (const fixture of fixturePairs) {
      ...
      const ours = dragonrouterPromptCompression(original);
