...
//      - request carries x-dragonrouter-session-id header
...
// reads "x-session-id" (not "x-dragonrouter-session-id") and prefixes the result with "ext:".
...
//     internal summary request (identified by _dragonrouterInternalRequest flag in body)
...
function installHandoffAwareFetch() {
  ...
  globalThis.fetch = async (url: any, init: any = {}) => {
    ...
    if (bodyObj._dragonrouterInternalRequest === "universal-handoff") {
...
// Without the x-dragonrouter-session-id header, sessionId = null → block is skipped.
test("context-relay universal handoff: does NOT fire when x-dragonrouter-session-id header is absent", async () => {
  ...
  assert.equal(
    handoff,
    null,
    "handoff must NOT be written when x-dragonrouter-session-id header is absent (sessionId gate)"
  );
