...
// `x-dragonrouter-no-memory` header (mirrors `x-dragonrouter-no-cache`).
...
test("isNoMemoryRequested is true for truthy header values (plain object)", () => {
  for (const v of ["true", "1", "yes", "TRUE", "Yes", " true "]) {
    assert.equal(
      isNoMemoryRequested({ "x-dragonrouter-no-memory": v }),
      true,
...
test("isNoMemoryRequested is case-insensitive on the header NAME", () => {
  assert.equal(isNoMemoryRequested({ "X-Dragon Router-No-Memory": "true" }), true);
});

test("isNoMemoryRequested works with a Headers instance", () => {
  const h = new Headers();
  h.set("x-dragonrouter-no-memory", "1");
...
test("isNoMemoryRequested is false when the header is absent / empty / falsy", () => {
  ...
  assert.equal(isNoMemoryRequested({ "x-dragonrouter-no-memory": "" }), false);
  assert.equal(isNoMemoryRequested({ "x-dragonrouter-no-memory": "false" }), false);
  assert.equal(isNoMemoryRequested({ "x-dragonrouter-no-memory": "0" }), false);
  assert.equal(isNoMemoryRequested({ "x-dragonrouter-no-memory": "no" }), false);
});

test("getHeaderValueCaseInsensitive still resolves the header (sanity)", () => {
  assert.equal(
    getHeaderValueCaseInsensitive({ "x-dragonrouter-no-memory": "true" }, "x-dragonrouter-no-memory"),
    "true"
  );
});
