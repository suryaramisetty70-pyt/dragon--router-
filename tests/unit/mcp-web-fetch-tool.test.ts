...
test("webFetchTool has the required McpToolDefinition shape", () => {
  assert.equal(webFetchTool.name, "dragonrouter_web_fetch");
...
test("dragonrouter_web_fetch is mapped in MCP_TOOL_SCOPES with execute:search", () => {
  const scopes = MCP_TOOL_SCOPES["dragonrouter_web_fetch"];
  assert.ok(scopes != null, "dragonrouter_web_fetch must have a scope mapping");
  assert.ok(
    scopes.includes("execute:search"),
    "dragonrouter_web_fetch must require execute:search scope"
  );
});
