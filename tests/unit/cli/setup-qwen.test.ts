...
test("buildQwenSettings adds an openai modelProvider (baseUrl /v1, envKey), sets model", () => {
  ...
  const p = s.modelProviders.find((x) => x.id === "dragonrouter");
  assert.equal(p.authType, "openai");
  assert.equal(p.baseUrl, "http://vps:20128/v1");
  assert.equal(p.envKey, "DRAGONROUTER_API_KEY");
  assert.equal(s.model, "glm/glm-5.2");
  assert.equal(s.selectedProvider, "dragonrouter");
});
test("buildQwenSettings de-dupes the dragonrouter provider + preserves others", () => {
  const s = buildQwenSettings(
    { modelProviders: [{ id: "other" }, { id: "dragonrouter", baseUrl: "old" }], theme: "dark" },
    { baseUrl: "http://x/v1", model: "m" }
  );
  assert.equal(s.modelProviders.filter((p) => p.id === "dragonrouter").length, 1);
