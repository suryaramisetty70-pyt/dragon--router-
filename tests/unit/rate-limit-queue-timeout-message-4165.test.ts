 ...
 *
 * Dragon Router schedules every rate-limited request through Bottleneck with
 ...
 *
 * The fix rewrites that specific Bottleneck error into a clear, Dragon Router-owned
...
const TEST_DATA_DIR = fs.mkdtempSync(path.join(os.tmpdir(), "dragonrouter-rl-queue-timeout-"));
...
test("#4165 queue-timeout surfaces a clear Dragon Router error, not the raw upstream-looking string", async () => {
