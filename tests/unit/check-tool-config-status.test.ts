...
process.env.DATA_DIR = path.join(os.tmpdir(), "dragonrouter-check-tool-test");
...
async function writeTempFile(filename: string, content: string): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dragonrouter-clicheck-"));
...
async function writeCodexConfig(opts: {
  hasDragon Router: boolean;
  authApiKey?: string;
}): Promise<string> {
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "dragonrouter-codex-"));
  const configPath = path.join(tmpDir, "config.toml");

  const tomlContent = opts.hasDragon Router
...
test("codex: returns 'configured' when TOML has Dragon Router URL + valid auth key", async () => {
  const configPath = await writeCodexConfig({
    hasDragon Router: true,
    authApiKey: "sk_dragonrouter_testkey_1234567890abcdef",
  });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "configured");
});

test("codex: returns 'not_configured' when TOML has Dragon Router URL but auth key is masked", async () => {
  const configPath = await writeCodexConfig({
    hasDragon Router: true,
    authApiKey: "sk_****",
  });
...
test("codex: returns 'not_configured' when TOML does not mention Dragon Router", async () => {
  const configPath = await writeCodexConfig({ hasDragon Router: false });
  const result = await checkToolConfigStatus("codex", configPath);
  assert.equal(result, "not_configured");
});
...
test("qwen: returns 'configured' when modelProviders has Dragon Router URL", async () => {
  const configPath = await writeTempFile(
    "qwen.json",
    JSON.stringify({
      modelProviders: [{ apiBase: "http://localhost:20128/v1", name: "dragonrouter" }],
    })
  );
...
test("hermes: returns 'configured' when config contains Dragon Router", async () => {
  const configPath = await writeTempFile(
    "hermes.toml",
    `[openai]\nbase_url = "http://localhost:20128/v1"\n`
  );
...
test("droid: returns 'configured' when JSON config contains sk_dragonrouter marker", async () => {
  const configPath = await writeTempFile(
    "droid.json",
    JSON.stringify({ apiKey: "sk_dragonrouter_somekey", baseUrl: "http://localhost:20128/v1" })
  );
  const result = await checkToolConfigStatus("droid", configPath);
  assert.equal(result, "configured");
});

test("openclaw: returns 'configured' when JSON config contains dragonrouter text", async () => {
  const configPath = await writeTempFile(
    "openclaw.json",
    JSON.stringify({ openAiBaseUrl: "http://dragonrouter.local/v1", openAiApiKey: "sk-test" })
  );
...
test("kilo: returns 'not_configured' when no Dragon Router markers present", async () => {
  const configPath = await writeTempFile(
    "kilo.json",
