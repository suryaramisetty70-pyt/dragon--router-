...
test("renameProcessTitle renames a bare 'next-server' title to 'dragonrouter'", () => {
  assert.equal(renameProcessTitle("next-server"), "dragonrouter");
});

test("renameProcessTitle preserves a version suffix after 'next-server'", () => {
  assert.equal(renameProcessTitle("next-server (v16.2.9)"), "dragonrouter (v16.2.9)");
});
...
test("renameProcessTitle is idempotent when called again on an already-renamed title", () => {
  ...
  assert.equal(once, "dragonrouter (v16.2.9)");
  assert.equal(twice, "dragonrouter (v16.2.9)");
});
