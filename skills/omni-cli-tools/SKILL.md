---
name: omni-cli-tools
description: "Manage CLI tool integrations exposed via the API. List, configure, and invoke CLI tool plugins that extend Dragon Router's automation surface."
---
...
Read Crush CLI Dragon Router config

Local-only. Reads the Dragon Router provider block in Crush's config.
...
Write Crush CLI Dragon Router config

Local-only. Registers Dragon Router as an `openai-compat` provider in Crush's config.
...
Remove Dragon Router from Crush CLI config

Local-only. Removes the Dragon Router provider block from Crush's config.
...
Read CodeWhale CLI Dragon Router config

Local-only. Reads the Dragon Router config block from `~/.codewhale/config.toml` (with `~/.deepseek/config.toml` legacy fallback).
...
Write CodeWhale CLI Dragon Router config

Local-only. Writes the Dragon Router config block in CodeWhale TOML format.
...
Remove Dragon Router from CodeWhale CLI config

Local-only. Removes the Dragon Router config block from CodeWhale's config.
