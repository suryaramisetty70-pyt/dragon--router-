...
description: "Interact with Dragon Router via JSON-RPC 2.0 agent-to-agent protocol. 6 built-in A2A skills: smart-routing, quota-management, provider-discovery, cost-analysis, health-report, list-capabilities."
---
...
<!-- skill:custom-start -->
<!-- Migrated from skills/dragonrouter-a2a/SKILL.md (preserved curated content) -->

# Dragon Router — A2A Protocol
...
Dragon Router publishes an Agent Card at `/.well-known/agent.json` and accepts
JSON-RPC 2.0 calls at `/a2a`.

## Discovery

```bash
curl $DRAGONROUTER_URL/.well-known/agent.json
```
...
```bash
curl -X POST $DRAGONROUTER_URL/a2a \
...
- `401` → missing/invalid `DRAGONROUTER_KEY`

## Reference

Full docs: https://github.com/diegosouzapw/Dragon Router/blob/main/docs/frameworks/A2A-SERVER.md
<!-- skill:custom-end -->
