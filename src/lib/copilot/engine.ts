/**
 * Dragon Router Copilot — Chat Engine
 *
...
function getKnowledgeResponse(query: string): string | null {
  ...
  if (
    ...
    return `## Dragon Router Architecture
...
Use \`createCombo\` tool or \`runDragonRouterCli\` to create them.`;
  }
...
Dragon Router supports 212+ providers across categories:
...
I can use CodeGraph to explore the Dragon Router codebase. Just ask me:
...
function getHelpResponse(): string {
  return `## Dragon Router Copilot — Comandos disponibles
...
### CLI
- "CLI health" → ejecuta \`dragon-router health\`
- "CLI list-combos" → ejecuta \`dragon-router list-combos\`
- "CLI set-budget 10" → ejecuta \`dragon-router set-budget 10\`

### Conocimiento
- "Cómo funciona Dragon Router?" → explica la arquitectura
...
export async function processCopilotChat(request: CopilotRequest): Promise<CopilotResponse> {
  ...
  if (!intent) {
    ...
    return {
      message: `I understand you want help with Dragon Router.\n\n${getHelpResponse()}`,
    };
  }
...
> Crea un combo llamado "fallback-claude" con estrategia priority y targets: [{"provider":"claude-code","model":"claude-sonnet-4"},{"provider":"openai","model":"gpt-4o"}]`,
  ...
  if (intent.tool === "runDragonRouterCli") {
    const tool = getCopilotTool("runDragonRouterCli");
    if (!tool) return { message: "Error: CLI executor not found." };

    const result = await tool.handler(intent.args);
    return {
      message: result,
      toolCalls: [{ name: "runDragonRouterCli", args: intent.args, result }],
    };
  }
