...
import { cooldownUntilMs } from "@dragon-router/open-sse/services/accountFallback.ts";
...
declare global {
  var __dragon_routerConnRecovery:
    | { initialized: boolean; interval: ReturnType<typeof setInterval> | null }
    | undefined;
}

function getRecoveryState() {
  if (!globalThis.__dragon_routerConnRecovery) {
    globalThis.__dragon_routerConnRecovery = { initialized: false, interval: null };
  }
  return globalThis.__dragon_routerConnRecovery;
}
...
function isRecoverySchedulerDisabled(): boolean {
  return (
    isEnvFlagEnabled("DRAGON_ROUTER_DISABLE_CONNECTION_RECOVERY") ||
    isEnvFlagEnabled("DRAGON_ROUTER_DISABLE_BACKGROUND_SERVICES") ||
    isBuildProcess() ||
...
/**
 * Resolve the tick interval (ms) from DRAGON_ROUTER_CONNECTION_RECOVERY_INTERVAL_MS,
...
export function resolveConnectionRecoveryIntervalMs(
  rawValue: string | undefined = typeof process !== "undefined"
    ? process.env.DRAGON_ROUTER_CONNECTION_RECOVERY_INTERVAL_MS
    : undefined
): number {
