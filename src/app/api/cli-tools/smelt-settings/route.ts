...
// POST — write Dragon Router settings to Smelt config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge Dragon Router settings (smelt uses OpenAI-compatible config)
...
// DELETE — remove Dragon Router settings from Smelt config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "Smelt Dragon Router settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
