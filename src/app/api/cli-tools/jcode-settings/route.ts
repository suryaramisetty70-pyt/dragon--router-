...
// POST — write Dragon Router settings to jcode config.json
export async function POST(request: Request) {
  ...
  try {
    ...
    // Merge Dragon Router settings (jcode uses OpenAI-compatible config)
...
// DELETE — remove Dragon Router settings from jcode config
export async function DELETE(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, message: "jcode Dragon Router settings removed" });
  } catch (err) {
    return NextResponse.json(
      { error: { message: sanitizeErrorMessage(err) } },
      { status: 500 }
