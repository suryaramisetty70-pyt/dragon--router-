...
/**
 * Rename a Node process title so Dragon Router is identifiable in `ps`/`htop`
...
export function renameProcessTitle(currentTitle: string): string {
  ...
  return `dragon-router${currentTitle.slice("next-server".length)}`;
}
...
export async function registerNodejs(): Promise<void> {
  // Rename the process title so Dragon Router is identifiable in ps/htop instead
  ...
  await import("@dragon-router/open-sse/index.ts");
  ...
  // See: https://github.com/diegosouzapw/Dragon Router/issues/3625 (Part A)
  try {
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    const { initBatchProcessor } = await import("@dragon-router/open-sse/services/batchProcessor");
    initBatchProcessor();
  ...
  try {
    ...
    if (settings.systemPrompt) {
      const { setSystemPromptConfig } =
        await import("@dragon-router/open-sse/services/systemPrompt.ts");
    ...
    const { hydrateThinkingBudgetConfig } = await import(
      "@dragon-router/open-sse/services/thinkingBudget.ts"
    );
  ...
  if (!isBackgroundServicesDisabled()) {
    ...
    try {
      const { autoRefreshDaemon } = await import("@dragon-router/open-sse/services/autoRefreshDaemon");
    ...
    // daemon on import (gated by DRAGON_ROUTER_ENABLE_LIVE_WS, default ON) — but NOTHING
    ...
    // ever pulled it into a module graph, so in the published `dragon-router` bin the
