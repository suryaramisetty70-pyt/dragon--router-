...
const LINUX_CERT_NAME = "dragon-router-mitm.crt";
  ...
  return new Promise((resolve) => {
    exec(
      script,
      {
        ...
        env: {
          ...process.env,
          CERT_NAME: "Dragon Router MITM Root CA",
          CERT_PATH: certPath || "",
          ACTION: action,
        },
      },
...
export async function installCert(sudoPassword: string, certPath: string): Promise<void> {
  ...
  if (process.env.DRAGON_ROUTER_SKIP_SYSTEM_TRUST === "1") {
    console.log("[cert] DRAGON_ROUTER_SKIP_SYSTEM_TRUST=1 — skipping OS trust-store mutation");
    return;
  }
...
// They install / remove the Dragon Router MITM root CA from the OS trust store and
